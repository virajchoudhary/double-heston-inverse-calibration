...
def baseline_reproduction(cells: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    checks["required_chart"] = "canonical"
    checks["passed"] = checks["canonical"]["passed"]
    return checks


def require_reproduced_160_baseline(reproduction: dict[str, Any]) -> None:
    """Stop before any 320 fit unless the required canonical 160 cell reproduces."""
    if not reproduction["passed"]:
        raise RuntimeError(f"reviewed 160 baseline failed before any 320 fit: {reproduction}")
...
def summarize_160_frames(
    ...
    return baseline_reproduction(cells)
...
def render_report(result: dict[str, Any]) -> None:
    ...
    reproduction_reason=(
        "The required canonical 160 cell reproduced the reviewed baseline: "
        ...
        "The transformed 160 cell changed slightly because it is now evaluated under the same corrected primitive-input "
        "reconstruction contract; that diagnostic change is recorded but is not the predeclared validity gate."
    )
    next_action={
...
def run() -> dict[str, Any]:
    ...
    require_reproduced_160_baseline(summarize_160_frames(frames,bounds))
    for chart in CHARTS:
