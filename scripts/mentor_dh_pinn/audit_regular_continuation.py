...
def audit_training_and_isolation(result, baseline):
    ...
    return {'selected_step': selection['step'], 'validation_iv_rmse': score,
            ...
            'hidden_layers': network.total_hidden_layers,
            'trainable_parameter_count': sum(p.numel() for p in network.parameters()),
...
def main():
    ...
    ap.add_argument('--audit-baseline-training', action='store_true',
                    help='Also verify a new paired control training run, rather than a historical baseline')
    args = ap.parse_args()
    torch.set_num_threads(1)
    args.out.mkdir(parents=True, exist_ok=False)
    baseline = collect(args.baseline)
    if args.audit_baseline_training:
        baseline['training_audit'] = audit_training_and_isolation(baseline, baseline)
    results = [baseline]
    ...
    fresh = all(r['manifest']['evidence_level']=='frozen assessment' for r in results)
    trained = [r for r in results if 'training_audit' in r]
    ...
    axes[1].set_xlabel('Fresh frozen case ID' if fresh else 'Previously exposed case ID'); axes[1].set_ylabel('Worst parameter error / its tolerance')
    axes[1].legend(fontsize=6); axes[1].grid(alpha=.2)
    fig.savefig(args.out/'recovery_comparison.png', dpi=160); plt.close(fig)
    paired = None
    if len(trained)==2:
        ca,cb=[r['training_audit']['training_config'] for r in trained]
        if ca.get('residual_blocks',0)!=cb.get('residual_blocks',0) and ca['checkpoint']==cb['checkpoint']:
            for k in ('seed','steps','batch','pde_batch','collocation','save_every','lr','min_lr',
                      'weight_decay','sensitivity','weight_pde','weight_shape','anchor_scale'):
                assert ca[k]==cb[k],f'Mismatched paired setting: {k}'
            ra,rb=[Path(r['manifest']['checkpoints'][0]['checkpoint']).parent for r in trained]
            for filename in ('fixed_training_state.npz','training_visits.npz'):
                a,b=np.load(ra/filename),np.load(rb/filename)
                assert a.files==b.files
                for k in a.files:np.testing.assert_array_equal(a[k],b[k])
            assert trained[0]['training_audit']['initial_validation_iv_rmse']==trained[1]['training_audit']['initial_validation_iv_rmse']
            paired={'same_parent_settings_sample_visits_and_fixed_physics_state':True,
                    'budget':'matched updates/data; not matched compute or independent initializations'}
    lines = ['# Regular-PINN architecture and training comparison', '',
        ('One-use fresh synthetic Double Heston comparison. Cases were predeclared before evaluation and were not used for training or selection. Twelve central-domain cases are not broad reliability evidence.' if fresh else
    ...
    for r in trained:
        ...
        lines.append(f'  Architecture: {a["hidden_layers"]} hidden layers, width {a["training_config"]["width"]}, {a["trainable_parameter_count"]:,} neural weights/biases (separate from the ten calibrated Heston parameters).')
    if paired:
        lines += ['', 'The shallow/deep pair has identical starting predictions, parent checkpoint, training settings, sampled-quote visit counts, collocation indices and fixed PDE weights. This comparison matches updates and data, not wall-clock time or compute. Neither arm is an independent restart.']
        fig,ax=plt.subplots(figsize=(8,4.2),layout='constrained')
        for r in trained:
            h=r['training_audit']['training_history']
            steps=[v['step'] for v in h]
            line,=ax.plot(steps,[v['validation_iv_rmse'] for v in h],label=r['label']+' validation')
            ax.plot(steps,[v['training_subset_iv_rmse'] for v in h],ls='--',color=line.get_color(),label=r['label']+' training subset')
        ax.set_xlabel('Additional AdamW updates');ax.set_ylabel('IV RMSE (absolute volatility)')
        ax.set_title('Training diagnostic versus validation-only selection');ax.grid(alpha=.2);ax.legend(fontsize=7)
        fig.savefig(args.out/'training_validation.png',dpi=160);plt.close(fig)
        lines += ['', '![Training and validation](training_validation.png)', '',
    ...
    table = ['# Latest checkpoint: all 120 parameter estimates', '',
             'Slow factor 1, fast factor 2. '+('One-use fresh frozen cases.' if fresh else 'Previously exposed development cases.'), '',
    ...
    (args.out/'audit.json').write_text(json.dumps({'results': results, 'paired_training_audit':paired,
