...
Implementation: `src/r2_representation/` (tests: `tests/test_r2_representation.py`, 44 tests)
Tracking issue: #25
...
- In-memory masks accept only genuine `bool` or `numpy.bool_` values.
  Strings (including `"True"` and `"False"`), integers, floats, `None`, and
  arbitrary truthy/falsy objects are rejected rather than coerced; JSON
  payloads likewise require JSON booleans.
...
- Caller-supplied `metadata` is an extension mechanism only. It is stored
  under `metadata["user_metadata"]`; the constructor reserves that namespace
  and never lets caller data overwrite authoritative fields
  (`synthetic`, `parameters_canonical_order`, `pricing_engine`, `node_count`,
  `target_moneyness_strikes`, `date_conditioning_id`, `expiry_dates`, `dte`,
  and `imputation`). A caller attempting to supply `user_metadata` directly
  is rejected.
...
| `metadata` | finite-number JSON-safe provenance object |
...
Metadata is validated at every construction and payload-read boundary: it
must be a mapping with string keys and contain only JSON-compatible
`null`, boolean, integer, floating-point, string, array/object structure.
Cycles, NaN, and either infinity are forbidden; unsupported Python objects
are not stringified or lossily converted. Excessively deep acyclic metadata
is rejected with the same contract error. An `R2Surface` stores this
normalized structure deeply immutably, so mutation of caller input or
returned nested provenance cannot change an existing surface.
...
A pre-merge contract review added three further boundaries: genuine
boolean-only in-memory masks; a reserved `user_metadata` namespace so
synthetic scientific provenance cannot be overwritten; and recursive,
finite-number JSON validation with deep immutability for stored metadata.

## Reproducing the contract checks

```bash
python -m pytest tests/test_r2_representation.py     # 44 focused tests
