...
The latest [PINN architecture ablation study](PINN_ARCHITECTURE_IMPROVEMENT_REPORT.md)
tests five continuation/correction variants across two seeds against the recovered
best checkpoint. The selected modified-MLP correction reduces fresh synthetic
price RMSE by 9.24% and passes all unchanged fidelity gates, but adds parameters
and inference cost and worsens some short-maturity and PDE errors. It remains a
research candidate; the existing default is unchanged. This numerical study does
not establish superiority over Single Heston or Black–Scholes on market data.

The September 18 [multiscale PINN extension and retrospective crisis study](experiments/nifty_multiscale_v5/REPORT.md)
adds a tested residual architecture and a matched-capacity ablation. The selected
extension reduces fresh synthetic price RMSE by 0.40%, but its restricted
Double-Heston family loses to calibrated Single Heston in all three market
windows overall. It beats the selected Black–Scholes baseline in two windows.
No guaranteed market superiority or ten-parameter recovery is claimed.
