...
| 9 | PDE physics | none yet (repricing consistency only) | PDE residual on pricer output | **RDD** (blocked on Node C) | **F19 CORRECTION:** B's PDE loss is INCORRECTLY implemented — autograd slice-view bug silently zeroes all variance-factor terms (implemented residual = d_tau − (diffusion + drift), a wrong PDE; identity-proven, `artifacts/diag_pde_bug_repro.py`). B PDE code: **DEPRECATE (bug; do not import)**. Even correctly wired, the residual is machine-zero for an accurate pricer → non-discriminating for parameters. A genuine Model-3 PDE-informed variant needs a fresh network-side PDE construction — an architecture decision, not a merge. |
