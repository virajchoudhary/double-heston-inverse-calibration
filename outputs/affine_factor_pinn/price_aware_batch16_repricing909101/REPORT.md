...
Interpretation: each cell is held-out price RMSE divided by 0.00001 of spot. Values at or below 1 meet that price threshold for the expiry; above 1 fail. An exclamation mark flags failure using the full-precision value, even when a label rounds to 1. The left panel uses learned prices; the right independently prices the SAME fitted parameters with the canonical Double Heston reference. Neither panel establishes correct parameter recovery. These per-expiry diagnostics do not replace or change the predeclared aggregate price gate.
