...
from data_hype import solve_f, parse, clean, forward_price, expiry_of
C = json.loads((HERE / 'config.json').read_text())


def test_parse_decimal_strike():
    assert parse('HYPE_USDC-25SEP26-40-C') == ('25SEP26', 40.0, 'C') and parse('HYPE_USDC-2OCT26-52d5-P') == ('2OCT26', 52.5, 'P')
...
def test_clean_on_synthetic_trades():
    base = 1_750_000_000_000; now = dt.datetime.fromtimestamp(base / 1000, dt.timezone.utc); F = 45.0; trades = []
    for code in ['20JUN25', '27JUN25', '25JUL25', '29AUG25', '26SEP25']:
        t = (expiry_of(code) - now).total_seconds() / (365 * 86400)
        for K in np.round(np.linspace(35, 56, 9), 1):
            cp = 'C' if K >= F else 'P'; p = F * float(forward_price(K / F, .7, t, cp == 'C'))
            if p < .004: continue
            trades.append({'instrument_name': f"HYPE_USDC-{code}-{str(K).replace('.', 'd')}-{cp}", 'timestamp': base, 'price': p, 'mark_price': p, 'iv': 70., 'index_price': F})
