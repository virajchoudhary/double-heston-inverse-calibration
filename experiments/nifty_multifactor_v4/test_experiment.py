...
def test_predeclared_state_domain_contains_all_scenario_extremes():
    lo,hi=C['pinn']['slow_state_domain'];a,b=C['pinn']['fast_state_domain']
    assert lo<=.0003*.8 and hi>=.06*.9*3.
    assert a<=.02*.1*.8 and b>=.06*3.
    from run import cases
    for row in cases(C):
        p=np.array(row['params']);admissible(p)
        assert lo<=p[4]<=hi and a<=p[9]<=b
