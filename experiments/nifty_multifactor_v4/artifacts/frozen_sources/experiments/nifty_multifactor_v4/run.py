"""Frozen, isolated experiment v4: PINN objective amendment, inheriting v3 artifacts. Run --help; old artifacts are read-only."""
import argparse
...
HERE=Path(__file__).resolve().parent; ROOT=HERE.parents[1]; OUT=HERE/'artifacts'; V3=HERE.parent/'nifty_multifactor_v3'
...
def freeze():
    ...
    sources=list(HERE.glob('*.py'))+list(HERE.glob('*.md'))+[HERE/'config.json']
...
INHERITED=['scenario_bank.json','market_validation','market_sources','market_validation_download.json','market_validation_status.json',
    'market_selection.json','controlled_cases.json','surfaces','baselines','teacher/train.npz','teacher/train_audit.json',
    'teacher/development.npz','teacher/development_audit.json','teacher/collocation.npz']
UNCHANGED_CONFIG=['published_double_slow_first','published_single','market_initial_volatilities','scale_rule','project_valid_analog_radius',
    'market_validation','market_final','market_data_policy','market_quote_filters','controlled_grid','families','independent_surfaces_per_family',
    'surface_sampling','representative_states_fast_slow','single_fit','bs','teacher_seeds','numerical','state_adaptive','statistics',
    'hypotheses','metrics','market_selection_objective','interpretation']
UNCHANGED_PINN=['architecture','depth','seeds','teacher_points','development_points','fidelity_test_points','collocation_points',
    'pde_test_points','adam_lr','label_batch','pde_batch','lbfgs_steps','lbfgs_labels','lbfgs_pde','scale_domain','slow_state_domain',
    'fast_state_domain','sampling','checkpoint','fidelity_gate','structural_claim_gate']
PROTOCOL_CODE=['run.py','literature_exact.py','test_experiment.py','report.py','AMENDMENT.md','CONTRACT_AUDIT.md','config.json']


def inherit():
    """v4: copy every v3 artifact that does not depend on the PINN objective, verifying SHA-256 of each file."""
    OUT.mkdir(exist_ok=False);c=read(HERE/'config.json');old=read(V3/'config.json');v3=V3/'artifacts';v3m=read(v3/'manifest.json')
    assert sha(v3/'manifest.json')==(v3/'manifest.sha256').read_text().strip()
    for k in UNCHANGED_CONFIG:assert c[k]==old[k],f'config {k} differs from v3'
    for k in UNCHANGED_PINN:assert c['pinn'][k]==old['pinn'][k],f'pinn {k} differs from v3'
    assert read(v3/'exact_validation.json')['passed'] and not v3m['market_final_opened']
    assert read(v3/'pinn_development_check/development_check.json')['decision']=='DO_NOT_LOCK_DOCUMENT_FAILURE'
    assert not (v3/'evaluation_lock.json').exists() and not (v3/'controlled_results.json').exists()
    assert sha(v3/'scenario_bank.json')==v3m['scenario_bank_sha256']
    records={}
    for rel in INHERITED:
        src=v3/rel;dst=OUT/rel;dst.parent.mkdir(parents=True,exist_ok=True)
        (shutil.copytree if src.is_dir() else shutil.copy2)(src,dst)
        for f in ([src] if src.is_file() else sorted(p for p in src.rglob('*') if p.is_file())):
            d=dst if src.is_file() else dst/f.relative_to(src);assert sha(f)==sha(d),f'copy mismatch {f}';records[str(f.relative_to(v3))]=sha(d)
    sources=[HERE/p for p in PROTOCOL_CODE]+[ROOT/p for p in ['src/double_heston.py','src/double_heston_reference.py','src/constraints.py',
        'src/mentor_dh_pinn/regular_pinn_torch.py','src/mentor_dh_pinn/regular_pinn_data.py','src/mentor_dh_pinn/torch_pricer.py',
        'scripts/mentor_dh_pinn/nifty_fixed_crisis.py','scripts/mentor_dh_pinn/audit_crisis_option_coverage.py']]
    snapshot=OUT/'frozen_sources';snapshot.mkdir()
    for p in sources:
    ...
    manifest={'frozen_utc':stamp(),'config':c,'scenario_bank_sha256':sha(OUT/'scenario_bank.json'),
        'source_hashes':{str(p.relative_to(ROOT)):sha(p) for p in sources},
        'inherited_from':{'experiment':'nifty_multifactor_v3','manifest_sha256':sha(v3/'manifest.json'),
            'v3_development_check_sha256':sha(v3/'pinn_development_check/development_check.json'),
            'v3_underfit_diagnosis_sha256':sha(v3/'pinn_development_check/underfit_diagnosis.json'),
            'v3_teacher_quality_sha256':sha(v3/'teacher_quality/teacher_quality.json')},
        'inherited_artifact_sha256':records,
        'development_exposure':v3m['development_exposure']+['v3 PINN development-split fidelity failure and training-data underfit diagnosis'],
        'market_final_opened':False,'convention':'literature_exact','canonical_contract_modified':False}
    save(OUT/'manifest.json',manifest);(OUT/'manifest.sha256').write_text(sha(OUT/'manifest.json')+'\n')
    print('V4 INHERITED',len(records),'files; manifest',sha(OUT/'manifest.json'),flush=True)
...
def fetch_market(stage):
    c=require_exact()
    if stage=='validation':assert not (OUT/'market_validation').exists(),'validation data are inherited from v3 and frozen'
    if stage=='final':
...
def market_select():
    assert not (OUT/'market_selection.json').exists(),'market selection is inherited from v3 and frozen'
...
def model(c,width=None):
    if width is None:
        selection=OUT/'pinn_selection.json'
        width=read(selection)['width'] if selection.exists() else c['pinn']['width']
    return TorchRegularVariancePINN(factors=2,width=width,depth=c['pinn']['depth'],tau_min=7/365,tau_max=2.,x_half_width=.36)
def structural(p):return tensor(np.stack([p[:,0:4],p[:,5:9]],axis=1))


def train(seed,candidate):
    c=require_exact();pc=c['pinn'];assert seed in pc['seeds'];spec=pc['candidates'][candidate];norm=pc['loss_normalisers'];torch.set_num_threads(1);torch.manual_seed(seed)
    assert not (OUT/'pinn_selection.json').exists(),'candidate already selected; no further training'
    dest=OUT/'candidates'/candidate/f'pinn_s{seed}';dest.mkdir(parents=True,exist_ok=False)
    net=model(c,spec['width'])
    ...
    def loss(di,ci):
        ...
        return parts[0]/norm['price']**2+parts[1]/norm['iv']**2+norm['pde_weight']*parts[2]/norm['pde']**2+norm['convexity_weight']*parts[3],parts
    opt=torch.optim.Adam(net.parameters(),lr=pc['adam_lr'])
    sched=torch.optim.lr_scheduler.CosineAnnealingLR(opt,T_max=spec['adam_steps'],eta_min=1e-5)
    started=time.monotonic();history=[]
    for step in range(spec['adam_steps']):
        ...
        if step%500==0 or step==spec['adam_steps']-1:
    ...
    opt.step(closure)
    final_value,final_parts=loss(di,ci)
    ...
    save(dest/'completed.json',{'utc':stamp(),'seconds':time.monotonic()-started,'history':history,
        ...
        'market_weight_updates':False,'structural_parameters_learned':False,'candidate':candidate,'spec':spec,'loss_normalisers':norm,
        'post_lbfgs_loss_on_lbfgs_subset':float(final_value.detach()),'post_lbfgs_parts':final_parts.detach().tolist()})
...
def candidate_nets(c,candidate):
    out=[]
    for seed in c['pinn']['seeds']:
        path=OUT/'candidates'/candidate/f'pinn_s{seed}';done=read(path/'completed.json')
        assert done['weights_sha256']==sha(path/'weights.pt') and done['manifest_sha256']==sha(OUT/'manifest.json')
        net=model(c,c['pinn']['candidates'][candidate]['width']);net.load_state_dict(torch.load(path/'weights.pt',weights_only=True));net.eval();out.append(net)
    return out


def select_candidate():
    """Predeclared ladder: lock the first candidate whose two-seed mean passes all fidelity gates on the development split."""
    c=require_exact();pc=c['pinn'];assert not (OUT/'evaluation_lock.json').exists() and not (OUT/'pinn_selection.json').exists()
    z=np.load(OUT/'teacher/development.npz');coords,p,y=z['coords'],z['params'],z['price'];x,tau=coords[:,0],coords[:,3];g=pc['fidelity_gate']
    rows=[];chosen=None
    for name in pc['candidate_order']:
        if not all((OUT/'candidates'/name/f'pinn_s{s}'/'completed.json').exists() for s in pc['seeds']):
            if chosen is None:raise RuntimeError(f'Predeclared candidate {name} must be trained before selection can continue')
            rows.append({'candidate':name,'status':'not_trained_not_needed'});continue
        pred,_=neural(candidate_nets(c,name),p,x,tau);m=metrics(y,pred,x,tau)
        ok=bool(m['price_RMSE']<=g['forward_price_RMSE_max'] and m['price_P95']<=g['forward_price_P95_max'] and m['price_max']<=g['forward_price_max_max']
                and m['IV_RMSE_volatility_points'] is not None and m['IV_RMSE_volatility_points']/100<=g['IV_RMSE_max'])
        row={'candidate':name,'status':'trained','development_gates_pass':ok,**m};rows.append(row)
        if chosen is None and ok:chosen=row;reason='first candidate in the predeclared order whose two-seed mean passes all development gates'
    if chosen is None:
        chosen=min((r for r in rows if r['status']=='trained'),key=lambda r:r['price_RMSE'])
        reason='NO candidate passes the development gates; lowest development price RMSE locked; held-out fidelity expected to fail and no structural claim is permitted'
    for s in pc['seeds']:shutil.copytree(OUT/'candidates'/chosen['candidate']/f'pinn_s{s}',OUT/f'pinn_s{s}')
    spec=pc['candidates'][chosen['candidate']]
    save(OUT/'pinn_selection.json',{'utc':stamp(),'rule':pc['selection_rule'],'chosen':chosen['candidate'],'reason':reason,
        'width':spec['width'],'depth':pc['depth'],'adam_steps':spec['adam_steps'],'development_results':rows,
        'checkpoint_sha256':{str(s):sha(OUT/f'pinn_s{s}/weights.pt') for s in pc['seeds']},'manifest_sha256':sha(OUT/'manifest.json')})
    print('PINN CANDIDATE SELECTED',chosen['candidate'],'|',reason,flush=True)
...
def lock():
    c=require_exact();assert (OUT/'pinn_selection.json').exists();nets(c)
    ...
    save(OUT/'evaluation_lock.json',{'utc':stamp(),'manifest_sha256':sha(OUT/'manifest.json'),
        ...
        'pinn_selection_sha256':sha(OUT/'pinn_selection.json')})
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('command',choices=['freeze','inherit','validate','fetch-validation','select-market','teacher','surfaces','baselines','train','select-candidate','lock','evaluate','fetch-final','market-final','report'])
    ap.add_argument('--seed',type=int,default=17);ap.add_argument('--candidate',default='C1');ap.add_argument('--shard',type=int,default=0);ap.add_argument('--shards',type=int,default=1)
    args=ap.parse_args();torch.set_num_threads(1)
    if args.command=='freeze':freeze()
    elif args.command=='inherit':inherit()
    ...
    elif args.command=='train':train(args.seed,args.candidate)
    elif args.command=='select-candidate':select_candidate()
