...
def main():
    c=verify();r=read(OUT/'controlled_results.json');table=pd.read_csv(OUT/'controlled_metrics.csv')
    ...
    report=f'''# Predeclared NIFTY/multifactor experiment — v3

v1 stopped at the exact-pricer short-time test before any fitting. v2 passed but its PINN domain missed analytic scenario extremes; v3 widens only that domain before training. The market bank and validation rule were unchanged. Its failed
record is preserved; see `../AMENDMENT.md` for the numerical integration fix.
...
Two fresh seeds (17,43), unchanged 5x128 smooth tanh architecture, float64,
100,000 synthetic labels, 18,000 PDE collocation points, 10,000 Adam steps with
...
Loss: normalized price MSE/.001² + valid IV MSE/.01² + .1 scaled PDE MSE/.01²
...
python run.py fetch-validation
python run.py select-market
python run.py surfaces
python run.py baselines
python run.py teacher
python run.py train --seed 17
