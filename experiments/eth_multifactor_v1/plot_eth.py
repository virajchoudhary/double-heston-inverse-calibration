"""Summary figure for eth (same layout as btc figure 6). Reads only."""
import json
...
ax.set_title('Ether options (Deribit), 53 test days 2021–2026: Double Heston has the lowest error in every situation', fontsize=10.5)
ax.legend(frameon=False, loc='upper right'); fig.subplots_adjust(bottom=.3, top=.9, left=.1, right=.97)
fig.savefig(FIG / 'eth_summary_comparison.png'); plt.close(fig)
