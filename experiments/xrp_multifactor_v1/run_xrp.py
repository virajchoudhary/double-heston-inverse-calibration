"""XRP multifactor replication. Commands, in order:
freeze -> validate -> select-dates -> fetch -> fit -> evaluate -> secondary
All rules live in config.json and were frozen before any XRP option surface was fetched.
...
SOURCES = ['config.json', 'PROTOCOL.md', 'data_xrp.py', 'run_xrp.py', 'test_xrp.py']
...
def freeze():
    ...
    m = {'frozen_utc': stamp(), 'config': c, 'source_sha256': {f: sha(HERE / f) for f in SOURCES}, 'btc_sha256': {f: sha(BTC / f) for f in BTC_SOURCES},
         ...
         'xrp_option_surfaces_fetched_before_freeze': False, 'models_fitted_before_freeze': False}
...
def validate():
    verify(); r = subprocess.run([sys.executable, '-m', 'pytest', '-q', str(HERE / 'test_xrp.py')], cwd=ROOT, capture_output=True, text=True)
...
def fetch():
    from data_xrp import fetch as get, clean
