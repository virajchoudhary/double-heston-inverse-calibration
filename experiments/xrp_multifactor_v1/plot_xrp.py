"""Summary figure for XRP (same layout as btc figure 6). Reads only."""
import json
...
ax.set_xticks(range(4), [c[2] for c in CELLS]); ax.set_ylim(0, 14.5); ax.set_ylabel('median pricing error on unseen options\n(implied-vol points, lower is better)')
ax.set_title('XRP options (Deribit), 26 test days 2024–2026: Double Heston clearly beats Black-Scholes; vs Single Heston it is mixed', fontsize=10.5)
ax.legend(frameon=False, loc='upper right'); fig.subplots_adjust(bottom=.3, top=.9, left=.1, right=.97)
fig.savefig(FIG / 'xrp_summary_comparison.png'); plt.close(fig)
